...
class RAGChatbot:
    ...
    def sample_function(self):
        return f"#### SAMPLE FUNCTION: {self.folder_path} #######"
    ...
    def chunk_list_of_documents(self,documents):
        """
    ...
    # def get_session_history(self,session_id: str):
        ...
        if self.session_id not in self.chat_history_store:
            self.chat_history_store[self.session_id] = ChatMessageHistory()
        return self.chat_history_store[self.session_id]

    def create_huggingface_retriever(self,folder_path,embedding_model_name):
        """
        ...
        print("loading pdfs...")
        documents_dict = self.load_pdfs(file_paths=files_paths)
        ...
        for key in tqdm(documents_dict.keys()):
            ...
            chunks = self.chunk_list_of_documents(documents=documents)
    ...
    def get_retrieved_documents_with_scores(self,vectorstore,query,k) -> List[Document]:
        """
    ...
    def instantiate_history_aware_retriever(self,retriever_hf,llm_model_name):
        """
    ...
    def setup_vector_db(self):
        ...
        self.session_id = "1" # hardcode this for temp fix since there is no persistence implemented

        # Create the Hugging Face retriever
        retriever_hf = self.create_huggingface_retriever(folder_path=self.folder_path,embedding_model_name=self.embedding_model_name)
        # Create the history-aware-retriever
        self.history_aware_retriever = self.instantiate_history_aware_retriever(retriever_hf=retriever_hf,llm_model_name=self.llm_model_name)

    def manual_rag_with_ollama(self,retrieved_documents, formatted_chat_history, input_query, ollama_model_name="llama3.1"):
        """
